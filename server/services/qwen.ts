...
4. Be warm and non-shaming — never use words like "should", "must", "failed", "overdue"
...
export async function generateNudge(db: Db, apiKey: string, baseURL?: string): Promise<string> {
...
- Last nudge: ${lastNudge[0]?.content ?? 'none'}`
  ...
  if (!text) throw new Error('Unexpected response from Qwen')
  return text
}
