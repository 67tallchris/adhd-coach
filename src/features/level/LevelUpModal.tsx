...
export function LevelUpModal() {
  ...
  const isNewTier = lastLevelUp.oldTier !== level.tier

  return (
    <>
      {/* Tier advancement celebration modal */}
        ...
        <div className="relative bg-gradient-to-br from-gray-900 to-gray-950 rounded-3xl border border-brand-700/50 w-full max-w-md p-8 overflow-hidden">
          ...
          <div className="relative z-10 text-center">
            ...
            <p className="text-gray-400 mb-6">
              {isNewTier 
                ? `You've reached ${newInfo.name} tier!`
                : `You're now level ${level.level}!`}
            </p>
            ...
            >
              {isNewTier ? 'See What\'s Next! 💪' : 'Keep Going! 💪'}
            </button>
          </div>
        </div>
      </div>

      {/* Next Stage Modal - shown after tier advancement */}
      {showNextStage && <NextStageModal onClose={() => setShowNextStage(false)} />}
    </>
  )
}
